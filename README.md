...
| `git restore --source [hash-commit] [file-name] ` | This command is similar to the following command:
`git checkout [commit-hash] [file-name]`
However, the difference is that the HEAD is not moved or detached. Essentially, we are telling Git to revert the specified file to a particular commit hash or source |
| `` |  |
| `` |  |
<br />
