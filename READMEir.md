...
| `git rm --cached --ignore-unmatch *.js` | تمامی فایلهای با پسوند مشخص شده را از مرحله ی استیج حذف می کند، اما فایلها در ورکینگ دایرکتوری باقی می مانند. |
| `git rm --cached [file name]` -> `git rm --cached .` | فایل مشخص شده را از استیج حذف می کند، اما فایل را از ورکینگ دایرکتوری پاک نمی کند. برای متوقف کردن ترک شدن فایل استفاده می شود. |
